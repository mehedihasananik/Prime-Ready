...
const
    Blogs = ({ item }) => {
        const { _id, name, image, description, quillValue } = item
        return (
            <Link href={`/dynamicblog/${_id}`} className=" bg-neutral-800">
                <div className="group relative cursor-pointer items-center justify-center overflow-hidden transition-shadow hover:shadow-xl hover:shadow-black/30">
                    <div className="">
                        <Image className="h-full w-full object-cover transition-transform duration-500 group-hover:rotate-3 group-hover:scale-125" width={400} height={400} src={image} alt="" />
                    </div>
                    ...
                    <div className="absolute inset-0 flex translate-y-[60%] flex-col items-center justify-center  text-center transition-all duration-500 group-hover:translate-y-0">
                        <h1 className="font-dmserif text-3xl font-bold text-white px-4">{name}</h1>
